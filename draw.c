...
void draw_polygons( struct matrix *polygons, screen s, color c ) {
  double x0, y0, x1, y1, x2, y2, ax, ay, bx, by;
  for(int i = 0; i < polygons->lastcol; i += 3){
    ...
    if( ax * by - ay * bx > 0){
      ...
      //draw_line(x2, y2, x3, y3, s, c);
      //draw_line(x3, y3, x1, y1, s, c);
    }
  }
}
...
void add_sphere( struct matrix * edges, 
  ...
  int index, lat, longt, p0, p1, p2, p3;
  ...
  for ( lat = latStart; lat < latStop; lat++ ) {
    for ( longt = longStart; longt < longStop; longt++ ) {
      ...
      p0 = index;
      p1 = (index + 1) % points->lastcol;
      p2 = (index + step) % points->lastcol;
      p3 = (index + step + 1) % points->lastcol;
      ...
      add_polygon( edges, points->m[0][p0],
		   ...
		   points->m[1][p2],
		   points->m[2][p2]);
      add_polygon( edges, points->m[0][p3], //this order matters, has to be (p3, p2, p1)
		   points->m[1][p3],
		   points->m[2][p3],
		   points->m[0][p2],
		   points->m[1][p2],
